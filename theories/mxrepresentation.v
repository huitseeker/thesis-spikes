...
Proof.
apply/subsetP=> x; case/setIdP=> Gx; rewrite mul1mx inE.
...
Proof.
move=> x Gx; rewrite gring_opE /gring_row rowK gring_indexK // mul1g.
...
Proof.
move=> A b; rewrite /gring_mx /= ![b *m _]mulmx_sum_row !linear_sum.
...
Proof. by move=> b; rewrite -[_ b]mul1mx gring_opJ gring_op1 mul1mx. Qed.
...
Proof. by move=> x; apply/matrixP=> i j; rewrite !mxE rmorph_nat. Qed.

Lemma map_gring_row : forall A : 'M_#|G|, (gring_row A)^f = gring_row A^f.
Proof. by move=> A; rewrite map_row. Qed.

Lemma map_gring_proj : forall x (A : 'M_#|G|),
  (gring_proj x A)^f = gring_proj x A^f.
Proof. by move=> x A; rewrite map_row -map_trmx map_gring_row. Qed.
...
Proof. by move=> a; rewrite map_vec_mx map_mxM map_enveloping_algebra_mx. Qed.

Lemma map_gring_op : forall A, (gring_op rG A)^f = gring_op rGf A^f.
Proof. by move=> A; rewrite map_gring_mx map_gring_row. Qed.
...
by case/submxP=> w ->; rewrite -mulmxA cUx.
Qed.

Lemma rstabs_act : forall x m1 (W : 'M_(m1, n)),
  x \in rstabs -> (W <= U)%MS -> (W *m rG x <= U)%MS.
Proof.
move=> x m1 W; case/setIdP=> _ nUx sWU; apply: submx_trans nUx.
exact: submxMr.
Qed.
...
Proof.
move=> m1 m2 U V; case/submxP=> u ->; apply/subsetP=> x; rewrite !inE.
...
by apply: submx_trans (mxmoduleP modU x Gx); exact: submxMr.
Qed.
...
Proof.
by move=> m W; rewrite -!submx0 -val_submodS linear0 !(submx0, eqmx0).
Qed.

Lemma in_submod_eq0 : forall m W, (@in_submod m W == 0) = (W <= U^C)%MS.
Proof.
move=> m W; apply/eqP/submxP=> [W_U0 | [w ->{W}]].
...
by case/submxP=> u ->; rewrite -!mulmxA copid_mx_id ?rank_leq_row.
Qed.
...
Proof.
by move=> m W; rewrite -!submx0 -val_factmodS linear0 !(submx0, eqmx0).
Qed.

Lemma in_factmod_eq0 : forall m (W : 'M_(m, n)),
  (in_factmod W == 0) = (W <= U)%MS.
Proof.
move=> m W; rewrite submxE -!mxrank_eq0 -{2}[_ U]mulmx_base mulmxA.
...
Proof.
move=> m W; case/submxP=> w ->{W}; rewrite /val_factmod /= -2!mulmxA.
...
move=> A; apply: (iffP submxP) => [[u defA] | [a ->]].
...
rewrite -{1}(add_proj_mx dxUV (submx_refl _)) !mulmx_addl addrC.
...
Proof. by move=> H; exact/rfix_mxP. Qed.
...
Proof.
move=> H sHG; apply/subsetP=> x; case/setIP=> Gx nHx; rewrite inE Gx.
...
Proof.
move=> H m U sHG; apply/subsetP/rfix_mxP=> cHU x Hx.
...
Proof.
move=> u; rewrite -!submx0; apply/idP/idP.
...
move=> u v; apply: (iffP sub_bigcapmxP) => [iso_uv | [f hom_uf <-] i _].
  have{iso_uv} uv0: forall A, (A \in E_G)%MS /\ u *m A = 0 -> v *m A = 0.
    move=> A; move/annihilator_mxP; case/submxP=> a defA.
...
case=> V; rewrite !genU; case/and4P=> modV sVU nzV ltVU; case: notF.
...
have phiG: forall x, x \in G -> phi *m rG x = rG x *m phi.
  move=> x Gx; rewrite -scalemxAl -scalemxAr; congr (_ *: _).
...
have tiUker: (U :&: kermx phi = 0)%MS.
  ...
  by case/submxP=> u ->; move/sub_kermxP; rewrite -mulmxA Uphi.
...
by move/negbFE: i0; rewrite -cyclic_mx_eq0; move/eqP->; exact: sub0mx.
Qed.
...
have [_ nzW _] := simW; apply: contra nzW; rewrite -!submx0.
exact: submx_trans (component_mx_id simW).
Qed.
...
Proof.
move=> m U; rewrite -row_full_dom_hom -sub1mx.
exact: submx_trans (submx1 _).
Qed.
...
case/submxP: Uu => v ->{u}; case/row_freeP=> u' vK; apply/row_subP=> i.
...
by rewrite E0 eqmx0 submx0 mxvec_eq0 -mxrank_eq0 mxrank1 n1.
Qed.
...
suffices: (1%:M <= V)%MS.
  by rewrite genmxE -(submxMfree _ _ f_free) mul1mx Vidf.
...
Proof.
rewrite /mx_absolutely_irreducible -!sub1mx; case/andP=> ->.
move/submx_trans; apply; apply/row_subP=> i.
...
Proof.
move=> H sHD; apply/eqmxP; apply/andP; split.
  by apply/rfix_mxP=> fx; case/morphimP=> x _ Hx ->; rewrite rfix_mx_id.
...
Proof.
move=> H sHG; apply/eqmxP; apply/andP; split; last first.
...
Proof.
move=> H sHG; apply/rfix_mxP=> x Hx.
...
Proof.
move=> H; apply/eqmxP; apply/andP; split.
...
Proof.
move=> K sKG; apply/eqmxP; apply/andP; (split; apply/rfix_mxP) => [x Kx | Hx].
...
Proof.
move=> M simM.
have simMG: forall g : [subg G], mxsimple rH (M *m rG (val g)).
...
case: (simM) => _; case/negP; rewrite -submx0.
...
apply: submx_trans (capmxSr U V); rewrite -in_factmod_eq0 linear_sub /=.
...
Lemma section_eqmx : forall U1 U2 V1 V2 modU1 modU2 modV1 modV2,
  ...
  mx_rsim (@section_repr U1 V1 modU1 modV1) (@section_repr U2 V2 modU2 modV2).
Proof. by move=> *; apply: section_eqmx_add => //; exact: adds_eqmx. Qed.
...
by apply: section_eqmx; rewrite -?rcons_cons nth_rcons ?leqW ?ltiU.
Qed.
...
Proof.
move=> U V ? ? ?; apply: section_eqmx; last by rewrite nth_rcons ltnn eqxx.
...
have maxV: max_submod V Um.
  move/max_submodP: (mx_rsim_irr simV irrV); move/(_ (submx1 _)).
...
Proof. by move=> m a M; rewrite vec_mxK submxMfree ?gring_free. Qed.

Lemma mem_sub_gring : forall m A (M : 'M_(m, nG)),
  (A \in M *m R_G)%MS = (A \in R_G)%MS && (gring_row A <= M)%MS.
Proof.
move=> m A M; rewrite -(andb_idl (memmx_subP (submxMl _ _) A)).
...
Proof. by move=> a; rewrite vec_mxK submxMl. Qed.

Lemma gring_opM : forall A B,
  (B \in R_G)%MS -> gring_op rG (A *m B) = gring_op rG A *m gring_op rG B.
Proof. by move=> A B R_B; rewrite -gring_opJ gring_rowK. Qed.
...
rewrite mulmx_addl mulNmx (sub_kermxP (val_submodP _)) oppr0 add0r.
...
rewrite linear_sum big1 ?scaler0 //= => y zGy.
...
by rewrite -!repr_mxM ?gring_projE ?groupM //; move/negPf->; rewrite scaler0.
Qed.
...
have: (gring_mx aG v \in R_G)%MS by rewrite vec_mxK submxMl.
...
rewrite big1 ?addr0 ?mxE /= => [ | y]; last first.
  ...
  by rewrite gring_projE ?groupM // eq_sym eq_mulgV1 mulgK (negPf nt_y) scaler0.
...
have sRS : R_S.
  by apply/sumsmx_subP=> i; rewrite genmxE submxMr ?(sumsmx_sup i).
...
Proof.
move=> i j ne_ij; apply/eqP; rewrite -submx0 capmxC.
...
Proof.
move=> i j ne_ij; apply/eqP; rewrite -submx0 -(Wedderburn_disjoint ne_ij).
...
Proof.
move=> i; rewrite /mxring /left_mx_ideal Wedderburn_closed submx_refl.
...
rewrite -submx0 in nzE; case/memmx_subP: nzE => A E_A.
...
Proof. move=> j A; rewrite eq_sym; exact: not_rsim_op0 rsim_irr_comp. Qed.
...
rewrite sub_capmx /= submx0 mxvec_eq0; case/andP=> R_A.
...
Proof.
move=> M modM iM simM sMiM; rewrite -[iM]irr_reprK.
symmetry; apply: irr_comp_rsim; apply/mx_rsim_iso.
by apply: component_mx_iso => //; exact: socle_simple.
Qed.

Lemma irr1_repr : forall x, x \in G -> irr_repr 1 x = 1%:M.
Proof.
move=> x Gx; suffices: x \in rker (irr_repr 1) by case/rkerP.
...
Proof.
move=> i; apply/eqP; rewrite -{1}[i]irr_reprK.
...
case/submxP=> b; move/(canRL mxvecK)=> ->{Br}; rewrite mulmxA mx_rV_lin /=.
...
Proof.
rewrite /irr_mode => x; case/setIP=> Gx cGx.
...
Proof.
elim=> [|n IHn] x Zx; first exact: irr_mode1.
...
by rewrite -irr_mode1 -(mulVg x) irr_modeM ?groupV // unitr_mul; case/andP=> _.
Qed.

Lemma irr_mode_neq0 : {in 'Z(G), forall x, irr_mode x != 0}.
Proof. by move=> x; move/irr_mode_unit; rewrite unitfE. Qed.
...
Proof. by move=> x Gx; rewrite /irr_mode irr1_repr ?mxE. Qed.
...
have sG'k: forall i : sGlin, G^`(1)%g \subset rker (irr_repr (val i)).
  by case=> i /=; rewrite !inE => lin; rewrite rker_linear //=; exact/eqP.
pose h' u := irr_comp sGq (quo_repr (sG'k u) nG'G).
have irrGq: forall u, mx_irreducible (quo_repr (sG'k u) nG'G).
  by move=> u; apply/quo_mx_irr; exact: socle_irr.
exists (fun i => oapp h' [1 sGq]%irr (insub i)) => [j | i] lin_i.
  rewrite (insubT (mem _) lin_i) /=; symmetry; apply/eqP; apply/socle_rsimP.
...
case: (pickP [pred x \in G | ~~ is_scalar_mx (rG x)]) => [x | scalG].
  case/andP=> Gx nscal_rGx; have: horner_mx (rG x) ('X^#|G| - 1) == 0.
    rewrite rmorph_sub rmorphX /= horner_mx_C horner_mx_X.
  ...
  rewrite row_free_unit rmorph_sub /= horner_mx_X horner_mx_C.
  rewrite (mx_Schur irrG) ?subr_eq0 //; last first.
    by apply: contra nscal_rGx; move/eqP->; exact: scalar_mx_is_scalar.
...
have: has #[x].-primitive_root (map r (enum sX)).
  ...
  by apply/allP=> rx; case/mapP=> W _ ->{rx}; rewrite unity_rootE rx1.
case/hasP=> w _ prim_w; exists w => //.
have iphi'P := prim_rootP prim_w (rx1 _); pose iphi' := sval (iphi'P _).
have def_r: forall W, r W = w ^+ iphi' W by move=> W; exact: svalP (iphi'P W).
...
pose iphi i := iinv (iphiP i); exists iphi.
have iphiK: cancel iphi iphi' by move=> i; exact: f_iinv.
have r_iphi: forall i, r (iphi i) = w ^+ i by move=> i; rewrite def_r iphiK.
...
Proof.
move=> m U; apply/setP=> x; rewrite !inE -!map_mxM inj_eq //; exact: map_mx_inj.
Qed.

Lemma rstabs_map : forall m (U : 'M_(m, n)), rstabs rGf U^f = rstabs rG U.
Proof. by move=> m A; apply/setP=> x; rewrite !inE -!map_mxM ?map_submx. Qed.
...
rewrite -map_mx_sub map_submx -in_factmod_eq0 linear_sub.
...
Proof.
split=> [x y|]; first by apply: mxval_inj; rewrite genK !rmorph_sub /= !genK.
...
Proof. by move=> k; rewrite rmorphX /= mxval_groot. Qed.
...
apply/eqP; apply/rowV0P=> v; rewrite sub_capmx; case/andP; case/submxP=> w.
...
Proof.
move=> m1 m2 U V sVA_V; apply/idP/idP=> siUV; last exact: submx_in_gen.
...
Proof.
move=> m1 U; apply/eqmxP; rewrite -submx_rowval_gen submx_refl /=.
...
case: (ltnP i n) => [lt_i_n | le_n_i]; last first.
  move=> _ -> //; exists F' => // G _; apply: splitF'; exact: leq_trans le_n_i.
